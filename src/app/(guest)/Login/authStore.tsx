...
export const useAuthStore = create(
    persist((set) => ({
        ...
        company: {},
        isCompanyLoggedIn: false,
        loginCompany: (data: any, callback: any) => {
            console.log(data)
            apiCall('company/login', data, {
                showLoader: true
            }).then((response) => {
                let responseData = handleResponse(response);
                if (responseData) {
                    if (responseData.status == true) {
                        set((state: any) => ({ ...state, company: responseData.data, isCompanyLoggedIn: true }));
                        toast({ message: responseData.message, status: 'success' });
                        let cookies = new Cookies();
                        let token = Math.random().toString(32).substring(2, 32)
                        cookies.set('company-auth', token);
                        callback?.success();
                    } else {
        ...
        logoutCompany: async () => {
            await apiCall('company/logout').then((response) => {
                let responseData = handleResponse(response);
                if (responseData) {
                    if (responseData.status == true) {
                ...
                set((state: any) => ({ ...state, company: {}, isCompanyLoggedIn: false }))
            });
        },
        // company auth
        admin: {},
